...
description: まとまった日本語の説明文・解説・記事・ドキュメントを書くとき、または「密度はあるが平坦で読み進める気がしない」「正しいのに退屈」な文章を診断・修正するときに使用する。短い報告・回答や箇条書き中心の応答には使わない。
---

# 認知リズムを生むための日本語ライティング規範

出典: k16shikano 氏による「認知リズム・ライティング」規範
(https://gist.github.com/k16shikano/eb2929f13ed19c97188393d297be8432)
を元に、本プラグイン向けに収録したもの。原文の「併用する規範」参照は本プラグインに含まれないため除いている。
...
- 適用対象は、まとまった分量の説明文・解説文・ドキュメントである。短い報告・回答・箇条書き中心の応答には適用しない。
- この規範と事実の正確さ・報告の簡潔さが衝突する場合は、正確さ > 簡潔さ > リズムの順で優先する。緊張の演出のために事実を後出しにしたり、必要な文脈を削ったりしない。
