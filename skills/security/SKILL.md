---
name: security
description: Security guidance and vulnerability review for codebases, APIs, services, CLI tools, libraries, and daemons. Use for security questions, focused reviews, vulnerability research, security audits, or pen tests. Run the complete workflow only for explicit codebase audit or pen-test requests, full/comprehensive/end-to-end reviews, or requested report artifacts. 日本語トリガー「セキュリティ監査して」「脆弱性を網羅的に探して」「セキュリティレポートを作って」や /jarvis:security の明示起動にも対応。単一の既知バグの原因調査には使わない。
---

# Security Audit

<!-- jarvis-local:start -->
## Jarvis integration notes (local addition)

This skill is vendored from [cloudflare/security-audit-skill](https://github.com/cloudflare/security-audit-skill) (MIT). See [UPSTREAM.md](UPSTREAM.md) for the pinned commit and sync procedure. Everything outside this `jarvis-local` block is upstream content; do not edit it directly.
...
### Authorization and artifact handling

- Run full audits only against repositories the user owns or is authorized to audit.
- Treat all audit artifacts as confidential vulnerability information. Never commit them to any repository, especially public ones, and never send them to external services.
<!-- jarvis-local:end -->
